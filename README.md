...
- 質問された際、AIは勝手にコードの修正を行うことを禁止します。質問の内容に答えることのみを優先してください。

### 環境変数 `APP_ENV`

PaperTerraceは、`APP_ENV`環境変数を使用して開発環境と本番環境で異なる動作を実現します。

#### 設定方法

バックエンドの環境変数として`APP_ENV`を設定します：
...
環境変数が設定されていない場合、デフォルトで`production`として動作します。

#### 開発環境と本番環境の違い

| 機能 | 開発環境 (`APP_ENV=development`) | 本番環境 (`APP_ENV=production`) |
|------|----------------------------------|--------------------------------|
| **検出要素の枠線** | オレンジ色の枠線を表示（デバッグ用） | 枠線を非表示（クリーンなUI） |
| **確信度スコア** | 各検出要素の左上に表示（例: Score: 95.3%） | 非表示 |
| **「AIに聞く」ボタン** | 対話型要素（図、表、数式など）にホバー時表示 | 対話型要素にホバー時表示 |

**対話型要素**: `table`, `figure`, `picture`, `formula`, `chart`, `algorithm`, `equation`

開発環境では、レイアウト解析の精度を視覚的に確認できるよう、全ての検出要素にオレンジの枠線と確信度が表示されます。本番環境では、ユーザー体験を最適化するため、これらのデバッグ情報は非表示になり、対話型要素の「AIに聞く」機能のみが提供されます。
