...
- **Backend**: Python 3.12+, FastAPI, SQLAlchemy (PostgreSQL), **uv** (Package Manager), **Ruff** (Lint)

### AI & Inference

- **Main AI Engine**: Google Gemini 2.5 Pro/Flash (DSPy 動的最適化)
- **Local Inference**: Dedicated `llama-cpp-python` bridge services
  - `inference-layout`: レイアウト解析・OCR
  - `inference-m2m100`: 多言語翻訳支援
  - `inference-qwen`: ローカル推論モデル連携
