...
class LayoutAnalysisService:
    ...
    def _initialize_model(self):
        ...
        self.session = ort.InferenceSession(
            self.model_path, providers=["CPUExecutionProvider"], sess_options=session_options
        )
        ...
        for i, input_info in enumerate(inputs):
            logger.info(
                f"  Input {i}: name='{input_info.name}', shape={input_info.shape}, type={input_info.type}"
            )

        logger.info(f"Model has {len(outputs)} outputs:")
        for i, output_info in enumerate(outputs):
            logger.info(
                f"  Output {i}: name='{output_info.name}', shape={output_info.shape}, type={output_info.type}"
            )
        ...
        if not self.image_input_name:
            ...
            logger.warning(
                f"Could not find image input, using first input: {self.image_input_name}"
            )
    ...
    def analyze_image(self, image_path: str | Path) -> list[LayoutItem]:
        ...
        for result in results:
            ...
            class_name = (
                self.LABELS[class_id] if class_id < len(self.LABELS) else f"Unknown({class_id})"
            )

            layout_items.append(LayoutItem(bbox=bbox, class_name=class_name, score=result["score"]))
    ...
    def _inference(
        self, preprocessed_img: np.ndarray, scale_factor: np.ndarray, im_shape: np.ndarray
    ) -> list:
        """ONNX推論実行"""
        ...
        try:
            ...
            if self.image_input_name:
                ...
                logger.debug(
                    f"Added image input: {self.image_input_name} with shape {preprocessed_img.shape}"
                )

            if self.scale_input_name:
                input_feed[self.scale_input_name] = scale_factor
                logger.debug(
                    f"Added scale input: {self.scale_input_name} with shape {scale_factor.shape}"
                )

            if self.im_shape_input_name:
                input_feed[self.im_shape_input_name] = im_shape
                logger.debug(
                    f"Added im_shape input: {self.im_shape_input_name} with shape {im_shape.shape}"
                )
    ...
    def _postprocess(
        ...
        if len(valid_predictions) > 0:
            ...
            logger.info(
                f"Coordinate range: X=[{min(x_coords):.1f}, {max(x_coords):.1f}], Y=[{min(y_coords):.1f}, {max(y_coords):.1f}]"
            )
        ...
        if results:
            ...
            logger.info(
                f"Final coordinate ranges: x=[{min(all_x1)}, {max(all_x2)}], y=[{min(all_y1)}, {max(all_y2)}]"
            )
