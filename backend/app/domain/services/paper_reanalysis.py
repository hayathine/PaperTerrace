...
from common.processing_status import PROCESSING, get_analysis_needs
...
async def trigger_pending_analyses(
    ...
    if needs.layout and include_layout:
        # ジョブ投入前に "processing" を設定して重複投入を防ぐ
        if storage:
            try:
                storage.update_processing_status(paper_id, "layout_status", PROCESSING)
            except Exception as e:
                log.warning(
                    "trigger_pending",
                    "layout_status を processing に設定できなかった",
                    paper_id=paper_id,
                    error=str(e),
                )
        ...
        if job_id:
            ...
            if storage:
                try:
                    storage.update_processing_status(paper_id, "layout_status", None)
                except Exception:
                    pass
    ...
    # -----------------------------------------------------------------------
    if needs.summary:
        # タスク起動前に "processing" を設定して重複起動を防ぐ
        if storage:
            try:
                ...
                log.warning(
                    "trigger_pending",
                    "summary_status を processing に設定できなかった",
                    paper_id=paper_id,
                    error=str(e),
                )
        asyncio.create_task(
    ...
    if needs.grobid and file_hash:
        ...
        if storage:
            try:
                ...
                log.warning(
                    "trigger_pending",
                    "grobid_status を processing に設定できなかった",
                    paper_id=paper_id,
                    error=str(e),
                )
        asyncio.create_task(
