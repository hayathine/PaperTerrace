...
const StampsLayer = {
    ...
    createPalette() {
        ...
        palette.className = 'fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-white rounded-full shadow-2xl border border-slate-200 p-2 flex space-x-2 transition-all duration-300 translate-y-24 opacity-0 z-50';
    ...
    selectStamp(stamp, btnElement) {
        ...
        Array.from(palette.children).forEach(c => {
            c.className = 'w-10 h-10 rounded-full flex items-center justify-center text-lg hover:bg-slate-100 transition-colors';
        });
        btnElement.className = 'w-10 h-10 rounded-full flex items-center justify-center text-lg bg-indigo-100 ring-2 ring-indigo-300 hover:bg-indigo-200 transition-colors';
    },
    ...
    toggleMode() {
        ...
        if (this.isStampMode) {
            ...
            stampBtn.classList.add('bg-indigo-100', 'text-indigo-600', 'ring-2', 'ring-indigo-300');
            readerView.style.cursor = 'crosshair';
            this.showToast('スタンプモード: タップしてスタンプを押せます');
        } else {
            palette.classList.add('translate-y-24', 'opacity-0');
            stampBtn.classList.remove('bg-indigo-100', 'text-indigo-600', 'ring-2', 'ring-indigo-300');
    ...
    attachEventListeners() {
        ...
        readerView.addEventListener('click', (e) => {
            ...
            let pageNumber = null;
            let x = e.clientX;
            let y = e.clientY + readerView.scrollTop; // Adjust for scroll? 
            ...
            // Save to backend
            this.saveStamp(this.paperId, this.selectedStamp, e.pageX, e.pageY, pageNumber);
    ...
    placeVisualStamp(x, y, symbol) {
        ...
        el.className = 'absolute text-2xl animate-bounce pointer-events-none select-none z-50 drop-shadow-md';
