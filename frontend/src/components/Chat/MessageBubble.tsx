...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, onStackPaper }) => {
    ...
    const renderContent = (content: string) => {
        ...
        while ((match = urlRegex.exec(content)) !== null) {
            ...
            const cleanUrl = targetUrl.substring(0, targetUrl.length - trailingPunctuation.length);
            const cleanLabel = labelText.substring(0, labelText.length - trailingPunctuation.length);
