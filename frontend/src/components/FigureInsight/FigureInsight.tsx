...
function getFigureId(figure: SelectedFigure): string {
	return (
		figure.id ||
		(figure.image_url ? `transient-${figure.image_url}` : "transient-unknown")
	);
}
	...
	return (
		...
		>
			...
			{figure.image_url ? (
				...
				<div className="relative w-full bg-slate-50 aspect-video flex items-center justify-center border-b border-slate-100">
					<div className="text-center text-slate-400">
						<svg
							className="w-10 h-10 mx-auto mb-1 opacity-40"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
						...
						<p className="text-[10px]">P.{figure.page_number}</p>
					</div>
					{figure.label && figure.label !== "image" && (
						<div className="absolute top-2 right-2 bg-orange-600/80 text-white text-[9px] font-bold px-2 py-0.5 rounded-full backdrop-blur-sm uppercase">
							{figure.label}
