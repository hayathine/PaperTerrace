...
const FigureInsight: React.FC<FigureInsightProps> = ({
	...
	const [loadingFigures, setLoadingFigures] = useState<Record<string, boolean>>(
		{},
	);
	...
	const handleExplain = async (figureId: string) => {
		setLoadingFigures((prev) => ({ ...prev, [figureId]: true }));
		try {
			const res = await fetch(`${API_URL}/api/figures/${figureId}/explain`, {
				method: "POST",
			});
			if (res.ok) {
				const data = await res.json();
				setFigures((prev) =>
					prev.map((fig) => {
						const fId = fig.id || fig.figure_id;
						if (fId === figureId) {
							return { ...fig, explanation: data.explanation };
						}
						return fig;
					}),
				);
			...
			console.error("[FigureInsight] Failed to explain figure", e);
		} finally {
			setLoadingFigures((prev) => ({ ...prev, [figureId]: false }));
		}
	};
								...
								{fig.explanation ? (
									...
									<div className="flex flex-col items-center justify-center py-2">
										<button
											type="button"
											onClick={() =>
												figId && !loadingFigures[figId] && handleExplain(figId)
											}
											disabled={!figId || !!loadingFigures[figId]}
