...
const PaperList: React.FC<PaperListProps> = ({ onSelectPaper, currentPaperId }) => {
    ...
    if (loading) return <div className="p-4 text-gray-500 text-xs animate-pulse">読み込み中...</div>;

    return (
        <div className="flex flex-col gap-2 p-2">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 px-2">Library</h2>
            {papers.length === 0 ? (
                <p className="text-xs text-gray-500 px-2">論文がありません</p>
            ) : (
                        ...
                        >
                            <div className="flex items-start gap-3">
                                <div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${
                                    currentPaperId === paper.paper_id ? 'bg-white animate-pulse' : 'bg-indigo-500'
                                }`} />
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className={`text-[10px] mt-1 ${
                                        currentPaperId === paper.paper_id ? 'text-indigo-100' : 'text-gray-500'
                                    }`}>
