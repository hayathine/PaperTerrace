...
interface PDFPageProps {
  ...
  evidenceHighlight?: { page: number; text: string; box_2d?: number[] } | null;
}
    ...
    return (
      <div
        ...
        style={{
          ...
          height: `${styleH}%`,
          willChange:
            isSelected || isJumpHighlight ? "transform, opacity" : "auto",
        }}
        onMouseDown={(e) => {
...
const PDFPage: React.FC<PDFPageProps> = ({
  ...
  const evidenceIndices = React.useMemo(() => {
    if (
      !evidenceHighlight ||
      evidenceHighlight.page !== page_num ||
      !words ||
      words.length === 0
    )
      return null;
  ...
  const isWordInEvidence = React.useCallback(
    (idx: number) => {
    ...
    >
      ...
      <div className="relative w-full">
        ...
        <div className="absolute inset-0 w-full h-full z-10">
          {words.map((w, idx) => {
            ...
            const isJumpHighlight =
              ...
              Math.abs((w.bbox[0] + w.bbox[2]) / 2 / width - jumpTarget.x) <
                0.005 &&
              Math.abs((w.bbox[1] + w.bbox[3]) / 2 / height - jumpTarget.y) <
                0.005;
                ...
                onClick={(i, word) => {
                  if (!isStampMode && !isAreaMode && onWordClick) {
                    if (selectionStart === selectionEnd && !selectionMenu) {
                      ...
                      const wordCenterX =
                        (word.bbox[0] + word.bbox[2]) / 2 / width;
                      const wordCenterY =
        ...
        <div className="absolute inset-0 w-full h-full z-20 pointer-events-none">
          {/* Evidence Highlight Box (for Integrated Analysis Grounding) */}
          {evidenceHighlight &&
            evidenceHighlight.page === page_num &&
            evidenceHighlight.box_2d && (
              <div
                className="absolute bg-emerald-400/30 border-2 border-emerald-500 rounded-lg shadow-[0_0_15px_rgba(52,211,153,0.5)] z-30 pointer-events-none animate-in fade-in zoom-in duration-300"
                style={{
                  left: `${evidenceHighlight.box_2d[1] / 10}%`,
                  top: `${evidenceHighlight.box_2d[0] / 10}%`,
                  width: `${(evidenceHighlight.box_2d[3] - evidenceHighlight.box_2d[1]) / 10}%`,
                  height: `${(evidenceHighlight.box_2d[2] - evidenceHighlight.box_2d[0]) / 10}%`,
                }}
              >
                <div className="absolute -top-6 left-0 bg-emerald-600 text-white text-[9px] font-bold px-2 py-0.5 rounded shadow-sm whitespace-nowrap">
                  {evidenceHighlight.text}
                </div>
              </div>
            )}
