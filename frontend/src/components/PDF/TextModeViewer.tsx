...
const TextModeViewer: React.FC<TextModeViewerProps> = ({
  ...
  const { token } = useAuth();
  const [fullText, setFullText] = useState<string | null>(propFullText || null);
  ...
  useEffect(() => {
    if (!paperId || propFullText) {
      // If we already have text via prop, don't fetch
      return;
    }
    ...
    const fetchText = async () => {
      ...
      try {
        ...
        setFullText(
          data.ocr_text || "No text content available for this paper.",
        );
      } catch (err: any) {
        console.error(err);
