    ...
    return (
        <div className="flex flex-col h-full bg-white font-sans">
            <div className="p-4 border-b border-slate-100 bg-slate-50">
                ...
                <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">Reading Stack</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                {papers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-40 text-slate-300">
                        ...
                        <p className="text-sm">URLをクリックしてスタックに追加できます</p>
                    </div>
                ) : (
                    papers.sort((a,b) => b.addedAt - a.addedAt).map((paper, idx) => (
                        <div key={idx} className="group relative bg-slate-50 hover:bg-white hover:shadow-md border border-slate-100 rounded-lg p-3 transition-all duration-200">
                            <div className="pr-6">
                                <a 
                                    href={paper.url} 
                                    target="_blank" 
                                ...
                                <p className="text-[10px] text-slate-400 mt-1">
                                    Added {new Date(paper.addedAt).toLocaleString()}
                                </p>
                            </div>
                            <button
