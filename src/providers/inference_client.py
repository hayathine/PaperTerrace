...
class InferenceServiceClient:
    ...
    def _record_failure(self):
        ...
        if self.failure_count >= self.failure_threshold:
            ...
            logger.error(f"推論サービスの回路ブレーカーを開きました（失敗回数: {self.failure_count}）")
    ...
    async def _make_request_with_retry(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                last_exception = e
                logger.warning(f"推論サービスリクエスト失敗 (試行 {attempt + 1}/{self.max_retries + 1}): {e}")
    ...
    async def analyze_layout(self, pdf_path: str, pages: Optional[List[int]] = None) -> List[Dict[str, Any]]:
        """レイアウト解析の実行"""
